...
<body>
	...
	<div id="main-div">
		<div id="left-div">
			<div>
				...
				<div id="audit-report" class="left-first">&emsp;📌审核</div>
				<div class="left-second">
					<ul>
		...
		<div id="right-div">
			<h3 id="title-h3" align="center">内容审核</h3>
		...
		$(document)
				.ready(
						function() {
												...
												columns : [
														...
														},
														{
															...
															formatter : function(
																	...
																	index) {
																if (row.status === 1) {
																	var element = "<input class='btn btn-sm btn-danger' onclick='hideVerify("
																			+ row.id
																			+ ","
																			+ row.status
																			+ ","
																			+ row.postId
																			+ ")' type='button' value='撤销处理'>"
																} else {
																	var element = "<input class='btn btn-sm btn-danger' onclick='hideVerify("
																			+ row.id
																			+ ","
																			...
																			+ ","
																			+ row.postId
																			+ ")' type='button' value='处理'>"
																			+ "&emsp;<input class='btn btn-sm btn-danger' onclick='ignoreVerify("
																			+ row.id
																			+ ")' type='button' value='忽视'>"
																}
							...
							hideVerify = function(id, value, postId) {
								var handle = "show";
								if (value == 0) {
									handle = 'hide'
								}
								var url = '/bbs/' + handle + '/verify/posts/'
										+ id + '/' + postId
								$.post(url, function(data) {
									$("#admin-table").bootstrapTable('refresh')
								})
							}
							ignoreVerify = function(id) {
								$.post('/bbs/ignore/posts/' + id,
										function(data) {
	...
	<script type="text/javascript">
				...
				.bootstrapTable(
						{
							...
							pageNumber : 1,
							search : true,
							queryParams : queryParamsReply,//请求服务器时所传的参数
							sidePagination : 'server',//指定服务器端分页
							pageList : [ 5,10,15],
							toolbar : '#toolbar',
							showRefresh:true,
							columns : [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											if (row.status === 1) {
												var element = "<input class='btn btn-sm btn-danger' onclick='hideVerifyR("
														+ row.id
														+ ","
														+ row.status
														+ ","
												...
												var element = "<input class='btn btn-sm btn-danger' onclick='hideVerifyR("
														...
														+ row.replyId
														+ ")' type='button' value='处理'>"
														+ "&emsp;<input class='btn btn-sm btn-danger' onclick='ignoreVerifyR("
														+ row.id
														+ ")' type='button' value='忽视'>"
											}
		...
		//请求服务数据时所传参数
		function queryParamsReply(params) {
			return {
				pageSize : params.limit, //每一页的数据行数，默认是上面设置的10(pageSize)
		...
		hideVerifyR = function(id, value, replyId) {
			var handle = "show";
			if (value == 0) {
				handle = 'hide'
			}
			var url = '/bbs/' + handle + '/verify/replys/' + id + '/' + replyId
			$.post(url, function(data) {
				$("#reply-table").bootstrapTable('refresh')
			})
		}
		ignoreVerifyR = function(id) {
			$.post('/bbs/ignore/replys/' + id, function(data) {
				$("#reply-table").bootstrapTable('refresh')
			})
		}
	</script>
